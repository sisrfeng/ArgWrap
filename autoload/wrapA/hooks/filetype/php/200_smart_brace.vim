...
function! wrapA#hooks#filetype#php#200_smart_brace#pre_wrap(range, container, arguments) abort " {{{
...
function! wrapA#hooks#filetype#php#200_smart_brace#pre_unwrap(range, container, arguments) abort " {{{
    " Do nothing but prevent the file to be loaded more than once
    " When calling an autoload function that is not define, the script that
    " should contain it is sourced every time the function is called
endfunction  " }}}

function! wrapA#hooks#filetype#php#200_smart_brace#post_wrap(range, container, arguments) abort " {{{
    if wrapA#getCfg('php_smart_brace')
        call s:fixMethodOpeningBraceAfterWrap(a:range, a:container, a:arguments)
    endif
endfunction  " }}}

function! wrapA#hooks#filetype#php#200_smart_brace#post_unwrap(range, container, arguments) abort " {{{
    if wrapA#getCfg('php_smart_brace')
